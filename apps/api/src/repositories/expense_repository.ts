...
export type ExpenseMethod = 'cash' | 'bank_transfer' | 'debit' | 'credit'
...
function mapRowToDto(row: any): ExpenseDto {
  return {
    id: row.id,
...
export const expenseRepository = {
  /**
   * Insert a new Expense as a draft. business_id is supplied by the caller
  ...
  /**
   * Find a single Expense scoped to the tenant. Soft-deleted rows are excluded.
  ...
  async findById(
    ...
    `
    const result = await client.query(sql, [expenseId, businessId])
    if (result.rows.length === 0) return null
    return mapRowToDto(result.rows[0])
  },

  /**
   * List Expenses for a tenant with optional filters and parameterized pagination.
  ...
  async list(
    ...
    const conditions = ['e.business_id = $1', 'e.deleted_at IS NULL']
    ...
    if (filters.search) {
      ...
      conditions.push(
        `(e.reference ILIKE $${paramIndex} OR e.description ILIKE $${paramIndex} OR e.category ILIKE $${paramIndex})`
      )
      params.push(pattern)
      paramIndex++
    }
  ...
  /**
   * Apply a partial patch to a DRAFT Expense with optimistic locking.
   * Returns the updated row, or null if the expense does not exist / is not a
  ...
  async updateDraft(
    ...
    const values: unknown[] = [expenseId, businessId, expectedServerVersion]
    let paramIndex = 4
  ...
  /**
   * Soft-delete a DRAFT Expense (never physical DELETE).
  ...
  /**
   * Sync query: find Expenses with server_version > afterVersion (includes
