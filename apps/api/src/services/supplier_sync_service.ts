import { Pool } from 'pg'
import { SupplierSyncListResponse } from '../dto/sync_dto'
...
export function createSupplierSyncService(pool: Pool) {
  return {
    async list(query: unknown, tenantId: string): Promise<SupplierSyncListResponse> {
      ...
      return withTransaction(pool, async (client) => {
        const rows = await supplierRepository.findByBusinessAfter(
          client,
          businessId,
          afterVersion,
          fetchLimit
