...
import { CustomerSyncListResponse } from '../dto/sync_dto'
...
export function createCustomerSyncService(pool: Pool) {
  return {
    async list(query: unknown, tenantId: string): Promise<CustomerSyncListResponse> {
      ...
      return withTransaction(pool, async (client) => {
        const rows = await customerRepository.findByBusinessAfter(client, businessId, afterVersion, fetchLimit)
