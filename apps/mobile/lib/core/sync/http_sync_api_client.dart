...
Map<String, dynamic> _saleDtoToBatchItem(SaleDto sale) {
  ...
  Future<PullExpensesResponse> pullExpenses({
    ...
    final uri = Uri.parse('$baseUrl/v1/expenses').replace(queryParameters: qp);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final json = jsonDecode(response.body) as Map<String, dynamic>;
        return PullExpensesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        final refresh = await _onRefresh?.call();
        if (refresh == RefreshResult.success) {
          return pullExpenses(
        ...
        throw HttpException(
          'Unauthorized while pulling expenses: session expired',
          statusCode: 401,
          requestId: response.headers['x-request-id'],
        );
      } else {
        throw HttpException(
          'Failed to pull expenses: HTTP ${response.statusCode}',
          statusCode: response.statusCode,
          requestId: response.headers['x-request-id'],
        );
      }
      ...
      if (e is HttpException) rethrow;
      throw NetworkException('Network error while pulling expenses', e);
    }
  }

  @override
  Future<ExpenseDto?> getExpense({required String id}) async {
    final uri = Uri.parse('$baseUrl/v1/expenses/$id');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw HttpException(
          'Unauthorized while getting expense: session expired',
          statusCode: 401,
          requestId: response.headers['x-request-id'],
        );
      } else {
        throw HttpException(
          'Failed to get expense: HTTP ${response.statusCode}',
          statusCode: response.statusCode,
          requestId: response.headers['x-request-id'],
        );
      }
      ...
      if (e is HttpException) rethrow;
      throw NetworkException('Network error while getting expense', e);
    }
  }
  ...
  Future<ExpenseDto> createExpense({
    ...
    final uri = Uri.parse('$baseUrl/v1/expenses');
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        throw HttpException(
          'Unauthorized while creating expense: session expired',
          statusCode: 401,
          requestId: response.headers['x-request-id'],
        );
      } else {
        String errorMsg = 'Failed to create expense: HTTP ${response.statusCode}';
        try {
      ...
      if (e is HttpException) rethrow;
      throw NetworkException('Network error while creating expense', e);
    }
  }
