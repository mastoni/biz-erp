...
import { getPlatformPlans, PLATFORM_PAGE_SIZE } from '@/features/platform/api';
import type { PlatformPlan } from '@/features/platform/types';
import {
  getApiErrorInfo,
  formatPlatformDate,
  formatNullable,
...
const FALLBACK = 'Terjadi kesalahan saat mengambil data paket.';
const PAGE_NOUN = 'paket';

const COLUMNS = ['Kode', 'Nama', 'Family', 'Tier', 'Siklus Tagihan', 'Tipe', 'Status', 'Dibuat', 'Diperbarui'];
...
export default function PlatformPlansPage() {
  ...
  const load = async (nextOffset: number) => {
    ...
    setRequestId(null);
    try {
      const data = await getPlatformPlans(PLATFORM_PAGE_SIZE, nextOffset);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-display font-bold tracking-tight text-ink">Paket</h2>
        <p className="text-ink/60 mt-1">Paket langganan platform</p>
      </div>
      ...
      {shouldShowError(loading, error) && (
        <div className="rounded-md bg-brick/5 border border-brick/20 p-4">
          <p className="text-sm text-brick font-medium">Gagal memuat data paket</p>
      ...
      {shouldShowEmpty(loading, error, items.length) && (
        <div className="rounded-md border-2 border-dashed border-ink/10 bg-card p-8 text-center">
          <p className="text-sm font-medium text-ink">Belum ada paket</p>
          <p className="text-sm text-ink/60 mt-1">Tidak ada data paket pada platform ini.</p>
        </div>
      )}
                ...
                <TableBody>
                  {items.map((row) => (
                    <TableRow key={row.code}>
                      ...
                      <TableCell className="text-ink/70">{formatNullable(row.family)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.tier)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.billing_cycle)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.type)}</TableCell>
