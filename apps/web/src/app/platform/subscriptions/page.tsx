...
import { getPlatformSubscriptions, PLATFORM_PAGE_SIZE } from '@/features/platform/api';
import type { PlatformSubscription } from '@/features/platform/types';
import {
  getApiErrorInfo,
  formatPlatformDate,
  formatNullable,
...
const FALLBACK = 'Terjadi kesalahan saat mengambil data langganan.';
...
export default function PlatformSubscriptionsPage() {
  ...
  const load = async (nextOffset: number) => {
    ...
    setRequestId(null);
    try {
      const data = await getPlatformSubscriptions(PLATFORM_PAGE_SIZE, nextOffset);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-display font-bold tracking-tight text-ink">Langganan</h2>
        <p className="text-ink/60 mt-1">Langganan lintas bisnis di platform</p>
      </div>
      ...
      {shouldShowEmpty(loading, error, items.length) && (
        <div className="rounded-md border-2 border-dashed border-ink/10 bg-card p-8 text-center">
          <p className="text-sm font-medium text-ink">Belum ada langganan</p>
          <p className="text-sm text-ink/60 mt-1">Tidak ada data langganan pada platform ini.</p>
        </div>
      )}
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {items.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell className="font-mono text-xs">{formatNullable(row.id)}</TableCell>
                      <TableCell className="font-mono text-xs">{formatNullable(row.business_id)}</TableCell>
                      <TableCell className="font-mono text-xs">{formatNullable(row.account_customer_id)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.plan_code)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.plan_family)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.status)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.billing_cycle)}</TableCell>
                      <TableCell className="text-ink/70">{formatCurrency(row.final_price, row.currency)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.currency)}</TableCell>
