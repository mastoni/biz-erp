...
import { getPlatformBundles, PLATFORM_PAGE_SIZE } from '@/features/platform/api';
import type { PlatformBundle } from '@/features/platform/types';
import {
  getApiErrorInfo,
  formatPlatformDate,
  formatNullable,
...
const FALLBACK = 'Terjadi kesalahan saat mengambil data bundle.';
const PAGE_NOUN = 'bundle';

const COLUMNS = ['Kode', 'Nama', 'Segmen', 'Instalasi', 'Status'];

export default function PlatformBundlesPage() {
  const [items, setItems] = useState<PlatformBundle[]>([]);
  ...
  const load = async (nextOffset: number) => {
    ...
    setRequestId(null);
    try {
      const data = await getPlatformBundles(PLATFORM_PAGE_SIZE, nextOffset);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-display font-bold tracking-tight text-ink">Bundle</h2>
        <p className="text-ink/60 mt-1">Bundle layanan platform</p>
      </div>
      ...
      {shouldShowError(loading, error) && (
        <div className="rounded-md bg-brick/5 border border-brick/20 p-4">
          <p className="text-sm text-brick font-medium">Gagal memuat data bundle</p>
      ...
      {shouldShowEmpty(loading, error, items.length) && (
        <div className="rounded-md border-2 border-dashed border-ink/10 bg-card p-8 text-center">
          <p className="text-sm font-medium text-ink">Belum ada bundle</p>
          <p className="text-sm text-ink/60 mt-1">Tidak ada data bundle pada platform ini.</p>
        </div>
      )}
                ...
                <TableBody>
                  {items.map((row) => (
                    <TableRow key={row.code}>
                      ...
                      <TableCell className="text-ink/70">{formatNullable(row.target_segment)}</TableCell>
                      <TableCell className="text-ink/70">{formatNullable(row.installation_required)}</TableCell>
